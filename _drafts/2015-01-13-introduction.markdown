...
- [RPL](https://tools.ietf.org/html/rfc6550)/[AODVv2](http://tools.ietf.org/html/draft-ietf-manet-aodvv2-06) <!-- TODO: how do we do this if the display node doesnt RIOT? -->
- [6LowPAN](http://en.wikipedia.org/wiki/6LoWPAN)
- [IEEE 802.15.4](http://en.wikipedia.org/wiki/IEEE_802.15.4)

## Hard- and Software
### Plant nodes
For the plant nodes, we chose to use the [SAM R21 boards](https://github.com/RIOT-OS/RIOT/wiki/Board%3A-Samr21-xpro) equipped with a [DFROBOT SEN0114 humidity sensor](http://www.dfrobot.com/index.php?route=product/product&product_id=599) because it is robust against oxidation caused by moist soil and approved for plant-monitoring.
...
### Display node
The display node is a [Raspberry Pi](http://www.raspberrypi.org). It manages all plants, collects their humidity statuses and posts changes to the web interface. To accomplish this, it serves as a web server and runs Californuim. All CoAP messages received through Californium are handled passed to the server if need be.  
